...
'''
def Robust_Quantum_fixedpoint_fast(b,A,maxite,tol_1 ,X_true,gamma,lamda,c,M,t,svd_sNo,shrink_para):
...
# Copyrights to Dr. Kezhi Li, @Imperial College, 22nd Jan. 2017
    ...
    while ~converged:
        ...
        rho1=np.reshape(rho,(d,d))
        result_1=(LA.norm(X_true-rho1,'fro'))^2/(LA.norm(X_true,'fro'))^2
    ...
    return rho1,result_min,result_rho,result_resid, t_095,fide_095

'''
main start    
'''
def project2Hermitian_singular_shrink_fast(rho ,alp,d,svd_sNo):
    # function that carries out the singular shrink step 
    # 
    # 
    #%   Copyrights to Dr. Kezhi Li, @Imperial College, 22nd Jan. 2017
    
    rho=np.reshape(rho,(d,d))
    # Hermitian matrix
    rho=(rho+rho.T)/2
